...
func _append_bounds(center : Vector3, size : Vector3, cell_size : Vector3, out_positions : PackedVector3Array, source_indices : PackedInt32Array, seen : Dictionary, source_idx : int) -> PackedVector3Array:
	...
	var ys := _axis_positions(center.y, size.y, cell_size.y) if fill_y_axis else PackedFloat32Array([center.y])
	...
	for x : float in xs:
		for y : float in ys:
			for z : float in zs:
				...
				if out_positions.size() >= max_points:
...
func execute(ctx : FlowData.EvaluationContext):
	...
	var out_data := FlowData.Data.new()
	if copy_input_attributes and in_data != null and in_data.size() > 0:
