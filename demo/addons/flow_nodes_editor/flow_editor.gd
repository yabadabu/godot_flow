...
func deleteGraphNodes( graph_nodes : Array[GraphNode] ):
	...
	for graph_node in graph_nodes:
		...
		if node:
			...
			# Remove only the scene nodes generated by the node being deleted.
			# Other spawners may remain clean during the incremental regeneration,
			# so clearing every generated node here would make their output vanish.
			if active_context and active_context.graph == current_resource:
				active_context.removeRegisteredInstancedNodes( node )
			else:
				removeGeneratedNodes( node.name )
